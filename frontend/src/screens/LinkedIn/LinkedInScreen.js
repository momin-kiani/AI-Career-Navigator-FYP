...
function LinkedInScreen() {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            await api.put('/linkedin/profile', formData);
            fetchProfile();
    ...
    const generatePost = async () => {
        try {
            const response = await api.post('/linkedin/generate-post', {
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
                ...
                {generatedPost && (
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        <pre className="whitespace-pre-wrap text-sm text-gray-700 mb-4">{generatedPost}</pre>
                        <button
